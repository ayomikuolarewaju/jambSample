...
export default async function DashboardPage() {
  ...
  const { data: session }      = registration
    ? await supabase.from('exam_sessions').select('*').eq('registration_id', registration.id).single()
    : { data: null }
      ...
      <main className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="bg-gradient-to-r from-green-700 to-green-600 rounded-2xl p-6 text-white animate-fade-in">
          ...
          <p className="text-green-200 text-sm mt-1">
            Reg No: <span className="font-bold text-white">{profile?.reg_number}</span>
            {profile?.state_of_origin && <span className="ml-3">📍 {profile.state_of_origin}</span>}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2">
            <ExamRegistrationCard userId={user.id} registration={registration} session={session}/>
          </div>
        </div>
        ...
        {session?.submitted_at && (
          <div className="card animate-slide-up">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-gray-700">Your Latest Result</h3>
              <Link href="/results" className="text-green-700 text-sm font-semibold hover:underline">View Full Result →</Link>
            </div>
            <div className="flex items-center gap-6">
              <div className="w-20 h-20 rounded-full border-4 border-green-700 flex items-center justify-center bg-green-50 flex-shrink-0">
                <div className="text-center">
                  <div className="text-xl font-black text-green-700">{Math.round(session.total_score ?? 0)}</div>
                  <div className="text-[10px] text-gray-400">/ 400</div>
                </div>
              </div>
              <div>
                <p className="text-2xl font-black text-gray-800">
                  {Math.round(session.total_score ?? 0)}<span className="text-gray-400 text-lg font-normal"> / 400</span>
                </p>
                <p className="text-sm text-gray-500 mt-1">
                  {session.is_auto_submitted ? '⏱ Auto-submitted (time expired)' : '✅ Manually submitted'}
                </p>
