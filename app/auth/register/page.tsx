...
// Collects email for retargeting but NO confirmation required.
// Login identifier = JAMB Reg Number + Password.
...
const COURSE_GROUPS = {
  Science: {
    icon: '🔬',
    description: 'Medicine, Engineering, Computer Science',
    electives: ['Mathematics', 'Physics', 'Chemistry', 'Biology', 'Agricultural Science'],
  },
  Commercial: {
    icon: '💼',
    description: 'Accounting, Business Admin, Economics',
    electives: ['Mathematics', 'Economics', 'Commerce', 'Accounting', 'Government'],
  },
  Arts: {
    icon: '🎨',
    description: 'Mass Communication, Linguistics, Law',
...
export default function RegisterPage() {
  ...
  // All fields in one flat object — single page, no steps
  const [form, setForm] = useState({
    fullName:      '',
    regNumber:     '',
    email:         '',   // collected for retargeting, not used for auth
    phone:         '',
    dateOfBirth:   '',
    gender:        '',
    stateOfOrigin: '',
  ...
  const toggleElective = (sub: string) => {
    setForm(f => {
      if (f.electives.includes(sub))
        return { ...f, electives: f.electives.filter(s => s !== sub) }
  ...
  const isValid =
    form.fullName && form.regNumber && form.phone &&
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Use reg number as the auth identifier (fake email format)
    // Real email is stored in profile metadata for retargeting
    ...
    const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
      ...
      options: {
        ...
        data: {
          ...
          contact_email:   form.email.trim(),   // stored in metadata for retargeting
    ...
    if (signUpError) {
      if (signUpError.message.includes('already registered')) {
        setError('This registration number already has an account. Please sign in.')
      } else {
    ...
    // 2. Sign in immediately — works when email confirmation is OFF in Supabase
    ...
    if (signInError) {
      // Likely email confirmation is still ON — tell user what to do
      setError(
        'Account created but could not sign in automatically. ' +
        'Go to Supabase → Authentication → Settings and disable "Enable email confirmations", then try again.'
      )
      setLoading(false)
      return
    }

    // 3. Save combination for dashboard to pick up
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-white py-8 px-4">
      <div className="max-w-2xl mx-auto animate-slide-up">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">Fill in your details and choose your subject combination</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="card space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. 12345678AB"
                  ...
                  required className="input-field uppercase" />
                <p className="text-[11px] text-gray-400 mt-1">This is your sign-in username</p>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 uppercase tracking-wide mb-1">
                  Email Address
                  <span className="ml-1 text-[10px] font-normal text-gray-400 normal-case">(optional – for notifications)</span>
                </label>
              ...
              <div>
                ...
                <div className="relative">
                  <input type={showPwd ? 'text' : 'password'} placeholder="Min. 6 characters"
                    ...
                    required minLength={6} className="input-field pr-10" />
          ...
          <div>
            <h2 className="font-bold text-gray-800 text-base mb-1 pb-2 border-b border-gray-100">
              📚 Subject Combination
            </h2>
            <p className="text-xs text-gray-500 mb-4">
              English Language is compulsory. Select your course group then pick 3 elective subjects.
            </p>
            ...
            {/* Compulsory */}
            <div className="flex items-center gap-2 p-3 rounded-lg bg-green-700 text-white text-sm mb-3">
              <CheckCircle size={14} />
                  ...
                  {COURSE_GROUPS[form.courseGroup].electives.map(sub => {
                    ...
                    return (
                      <button type="button" key={sub} onClick={() => toggleElective(sub)} disabled={maxed}
                        className={clsx(
                          'flex items-center gap-2 p-3 rounded-lg border-2 text-xs text-left transition-all',
